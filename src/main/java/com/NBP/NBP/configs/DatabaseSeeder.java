...
public class DatabaseSeeder {
    ...
    CommandLineRunner seedDatabase(
        ...
        return args -> {
            ...
            // Seed laboratories
            seedFromCSV("seedData/laboratories.csv", line -> {
                try {
                    String name = line[0];
                    int departmentId = Integer.parseInt(line[1]);
                    Laboratory lab = new Laboratory();
                    lab.setName(name);
                    lab.setDepartmentId(departmentId);
                    laboratoryService.saveLaboratory(lab);
                } catch (Exception e) {
                    System.err.println("Skipping laboratory due to error: " + Arrays.toString(line));
                    System.err.println("Reason: " + e.getMessage());
                }
            });
            ...
            // return;
            // }
            // supplierService.saveSupplier(new Supplier(supplierName,
            // equipment.get().getId()));
            // } catch (Exception e) {
            // System.err.println("Skipping supplier due to error: " +
            // Arrays.toString(line));
            // System.err.println("Reason: " + e.getMessage());
            // }
            // });
            ...
            // .orElseThrow(
            // () -> new RuntimeException("User with username '" + username + "' not
            // found."));
            ...
            // department.getId()));
            // } catch (Exception e) {
            // System.err.println("Skipping custom user due to error: " +
            // Arrays.toString(line));
            // System.err.println("Reason: " + e.getMessage());
            // }
            // });
            ...
            // new Rental(equipment.get().getId(), LocalDate.parse(line[1]),
